...
test('ochrona przycisku ignoruje drugie kliknięcie', async () => { let release;const client={generate:()=>new Promise(r=>{release=r})};const handler=createGenerateHandler(client,async()=>prepared);const first=handler();const second=await handler();assert.deepEqual(second,{ignored:true});release({documentUrl:'x'});await first; });
