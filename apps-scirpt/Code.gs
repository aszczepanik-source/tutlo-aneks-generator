...
function doPost(event) {
  ...
  const isFormRequest = Boolean(
    event &&
    event.parameter &&
    typeof event.parameter.payload === 'string'
  );
  let request;
  ...
  try {
    ...
    if (isFormRequest) {
      return HtmlService.createHtmlOutput(
        '<!doctype html>' +
        '<html><head><meta charset="utf-8"><title>Przekierowanie</title></head>' +
        '<body><p>Dokument został wygenerowany. Trwa przekierowanie…</p>' +
        '<script>window.location.replace(' + JSON.stringify(result.documentUrl) + ');</script>' +
        '</body></html>'
      );
    }
    ...
    if (isFormRequest) {
      ...
      return HtmlService.createHtmlOutput(
        '<!doctype html>' +
        '<html><head><meta charset="utf-8"><title>Błąd generowania</title></head>' +
        '<body><p>Nie udało się wygenerować dokumentu: ' + message + '</p></body></html>'
      );
    }
