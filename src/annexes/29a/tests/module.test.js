...
import { prepareAnnex29a } from '../generator.js';
...
test('aneks 29a: oblicza cenę i wszystkie placeholdery wyłącznie z currentContract', () => {
  const prepared = prepareAnnex29a({ ...contract, rawText: new Proxy({}, { get() { throw new Error('rawText read'); } }) }, { today: '2026-07-28' });
  assert.equal(prepared.calculation.newCoursePriceCents, 1276000);
...
test('aneks 29a: mapuje wyłącznie PESEL osoby lub NIP firmy w obu trybach', () => {
  const cases = [
  ...
  for (const { mode, contract: modeContract, options } of cases) {
    const person = prepareAnnex29a({ ...modeContract, customerType: 'person', personalId: '12345678901' }, { ...options, mode, today: '2026-07-28' });
    const company = prepareAnnex29a({ ...modeContract, customerType: 'company', personalId: '1234567890' }, { ...options, mode, today: '2026-07-28' });
...
test('aneks 29a: jutro uwzględnia koniec miesiąca, roku i rok przestępny', () => {
  assert.equal(prepareAnnex29a(contract, { today: '2026-12-31' }).values.DATA_WEJSCIA_W_ZYCIE, '01.01.2027');
  assert.equal(prepareAnnex29a(contract, { today: '2028-02-28' }).values.DATA_WEJSCIA_W_ZYCIE, '29.02.2028');
});

test('aneks 29a: dopuszcza tylko flexible/internal/internal_24 i dodatnią nową cenę', () => {
  ...
  assert.ok(validate({ ...contract, coursePriceCents: 2 * contract.monthlyInstallmentCents }).length);
  assert.ok(validate({ ...contract, monthlyInstallmentCents: undefined }).length);
});

test('aneks 29a: rzeczywisty DOCX ma komplet placeholderów i generuje treść bez podwójnego zł', async () => {
  const prepared = prepareAnnex29a(contract, { today: '2026-07-28' });
...
test('aneks 29a: DOCX firmy zawiera wyłącznie etykietę NIP i nie pozostawia PESEL', async () => {
  const template = await readFile(new URL('../template.docx', import.meta.url));
  const prepared = prepareAnnex29a({ ...contract, customerType: 'company', personalId: '1234567890' }, { today: '2026-07-28' });
...
test('aneks 29a: manifest wskazuje istniejący szablon i właściwy wariant', () => {
