...
export function prepareAnnex45C(currentContract, inputs = {}, annexDate = localIsoDate(new Date())) {
  ...
  const agreementDate = currentContract.agreementDate ?? currentContract.dataZawarciaUmowy ?? currentContract.contractDate;
  const values = {
  ...
  const summary = {
    annex: '45C – Zmiana raty i limitu tygodniowego', customer: values.IMIE_NAZWISKO,
    agreementNumber: values.NUMER_UMOWY, agreementDate: values.DATA_ZAWARCIA_UMOWY,
    ...
    newPrice: values.NOWA_CENA, newAverageInstallment: values.NOWA_SREDNIA_RATA, weeklyLimit: limit,
    annexDate: values.DATA_ANEKSU, effectiveDate: values.DATA_WEJSCIA_W_ZYCIE,
    firstChangedMonth: formatDate(calculation.effectiveDate).slice(3)
  };
